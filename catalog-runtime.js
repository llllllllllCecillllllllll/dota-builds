(function(){
  ...
  function patch(){
    ...
    window.itemChoices=function(){
      ...
      var base=[];
      if(cat==='Все')base=originalItemChoices();
      else if(cat!=='Нейтральные'&&cat!=='Зачарования')base=originalItemChoices();
      ...
      if(cat==='Все')return all+html(extraFiltered);
      if(cat==='Нейтральные'||cat==='Зачарования')return html(extraFiltered);
      return all;
    };
  }
  ...
  function refreshGrid(){
    patch();injectCategories();
    var grid=document.getElementById('itemGrid');
    if(grid&&typeof window.itemChoices==='function')grid.innerHTML=window.itemChoices();
  }
  function loadCatalog(){
    var s=document.createElement('script');
    s.src='assets/catalog.js';
    s.onload=function(){
      fetch('assets/enchantments.json',{cache:'no-store'}).then(function(r){return r.ok?r.json():[];}).then(function(x){window.DOTA_ENCHANTMENTS=x||[];refreshGrid();}).catch(function(){refreshGrid();});
    };
    s.onerror=function(){refreshGrid();};
    document.head.appendChild(s);
  }
  function start(){
    patch();
    injectCategories();
    if(document.getElementById('itemGrid'))refreshGrid();
    loadCatalog();
  }
