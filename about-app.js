...
const DATES={'1.05':'28.08.2026','1.04':'27.08.2026','1.03':'26.08.2026','1.02':'26.08.2026','1.01':'26.08.2026'};
...
function render(inner){
  ...
  if(!root)return;
  root.innerHTML='<div class="modal"><div class="sheet about-sheet">'+inner+'</div></div>';
}
function header(title,subtitle){
  return '<div class="sheet-head"><div><h3>'+esc(title)+'</h3><div class="muted">'+esc(subtitle)+'</div></div><button type="button" class="close" data-about-close>Закрыть</button></div>';
}
function showAbout(){
  const dark=getTheme()==='dark';
  render(header('О приложении','Dota Assistant')+
    '<div class="about-card">'+
      '<div class="about-row"><span>Версия:</span><strong>'+esc(VERSION)+'</strong></div>'+
      '<div class="about-row"><span>Разработчик:</span><strong>'+esc(DEVELOPER)+'</strong></div>'+
      '<div class="about-row"><span>Дата версии:</span><strong>'+esc(CREATED)+'</strong></div>'+ 
    '</div>'+ 
    '<button type="button" class="theme-toggle" data-theme-toggle><span>Тема интерфейса</span><span class="theme-state">'+(dark?'Тёмная':'Светлая')+' <span class="theme-icon">'+(dark?'☾':'☀')+'</span></span></button>'+ 
    '<button type="button" class="about-link" data-show-versions><span>Версии и изменения</span><span>›</span></button>'
  );
}
function showVersions(){
  const versions=Object.entries(CHANGES).map(function(entry){
    const v=entry[0], changes=entry[1];
    return '<div class="version-block"><div class="version-head"><strong>v'+esc(v)+'</strong><span>'+esc(DATES[v]||'')+'</span></div><ul>'+changes.map(function(x){return '<li>'+esc(x)+'</li>';}).join('')+'</ul></div>';
  }).join('');
  render(header('Версии','История изменений')+versions);
}

document.addEventListener('click',function(e){
  if(e.target.closest('[data-theme-toggle]')){applyTheme(getTheme()==='dark'?'light':'dark');showAbout();return;}
  if(e.target.closest('[data-show-versions]')){showVersions();return;}
  if(e.target.closest('[data-about-close]')){const root=document.getElementById('modal');if(root)root.innerHTML='';}
});
